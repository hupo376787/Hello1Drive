...
- OneDrive 根目录、子目录浏览、面包屑导航、返回、根目录、刷新；手机端目录使用固定逻辑槽位：已知 `folder.childCount` 时首屏立即建立完整滚动范围，首个 children 页面先显示，其余 metadata 在后台连续填充占位槽位，滚动本身不再触发“加载更多”。同时维护按账户隔离的本地 metadata 索引并用 Microsoft Graph delta 后台同步，已索引目录再次进入可直接从本地显示。最近访问目录仍保留内存缓存和滚动位置恢复。
- 当前目录搜索。
...
- 三种视图：**详细信息 / 大图标 / 超大图标**。
- 详细信息列支持名称、大小、修改时间排序；设置中可指定全局默认排序（系统默认/日期/名称/大小升降序），每次修改全局默认都会清空并覆盖全部文件夹旧的独立排序。之后每个文件夹仍可单独覆盖；排序菜单中的“系统默认”会清除当前文件夹的独立排序并恢复设置中的全局默认。类型列仅显示，不提供排序。大小排序会为兼容部分 OneDrive 后端自动附加非索引查询 Prefer；若后端仍返回 `SMTotalFileStreamSize` 501，则仅该目录回退系统默认顺序。
...
- 已打开文件使用本地持久缓存；再次打开时优先使用目录列表携带的版本标识校验，文件未变化则直接使用缓存，变化后才重新下载。图片/视频缩略图也使用独立的持久磁盘缓存：Desktop 位于程序目录 `cache/thumbnails`，移动端位于应用数据目录；重新启动后只要文件版本未变化，就直接从本地缩略图缓存显示，不再重新下载缩略图。设置中的“清除缓存”会同时清除原文件缓存、缩略图缓存和当前账户的本地 metadata 索引，然后从 Graph 重新建立当前目录。
...
- 文件夹进入只把首个 children 请求放在关键路径；已知 childCount 时手机端立即建立完整虚拟槽位，后续 Graph 页面与滚动解耦并后台连续填充。childCount 缺失时后台补齐；本地 metadata 索引可直接恢复完整目录，Graph delta 在后台同步云端变化。详见 `docs/LOCAL_INDEX_SYNC.md`。
